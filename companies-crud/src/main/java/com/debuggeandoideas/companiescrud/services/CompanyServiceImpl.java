...
public class CompanyServiceImpl implements CompanyService {
    ...
    @Override
    public Company update(Company companys, String name) {
        var companyToUpdate = this.companyRepository.findByName(name).orElseThrow(()-> new NoSuchElementException("Company not found"));
        companyToUpdate.setLogo(companys.getLogo());
        companyToUpdate.setFoundationDate(companys.getFoundationDate());
        companyToUpdate.setFounder(companys.getFounder());
